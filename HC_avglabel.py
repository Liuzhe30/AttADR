...
def data_avg(X):
    ...
    for key in avg_dict.keys():
        new_line = []
        length = len(avg_dict[key])
        print(length)
        new_array = np.array(avg_dict[key])
        print(new_array.shape)
        sum_array = np.sum(new_array, axis = 0)
        print(sum_array.shape)
        for item in sum_array:
            new_line.append(float(item)/length)
        new_line.append(key)
        new_matrix.append(new_line)
    
    X_avg = np.array(new_matrix)
...
#Z = linkage(X, 'ward')
Z = linkage(X_avg, method='single', metric=mydist, optimal_ordering=False)
